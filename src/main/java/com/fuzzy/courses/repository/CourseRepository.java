...
public interface CourseRepository extends JpaRepository<Course, Long> {
    ...
    @Query(
            value = """
                    SELECT
                                            		c.id as course_id,
                                                DATE_ADD(c.start_date, INTERVAL 30 DAY) AS end_date,
                                                CASE
                                                    WHEN :today < c.start_date THEN 'Agendado'
                                                    WHEN :today > DATE_ADD(c.start_date, INTERVAL 30 DAY) THEN 'Encerrado'
                                                    WHEN :today BETWEEN c.start_date AND DATE_ADD(c.start_date, INTERVAL 30 DAY) THEN 'Em andamento'
                                                END AS status,
                                            ...
                                            FROM
                                                courses c
                                            LEFT JOIN
                                                courses_collaborators ce ON c.id = ce.course_id
                                            WHERE
                                                c.id = :id
                                            GROUP BY c.id;
                    """,
